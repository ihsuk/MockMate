const SessionCard = ({ session, onClick, onDelete }) => {
    ...
    const statusColor = session.status === 'completed' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : session.status === 'in-progress' ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400' : 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400';

    const iconBg = session.status === 'completed' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-500' : 'bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-500';

    const scoreColor = session.status === 'completed' ? (session.overallScore > 75 ? ' text-emerald-500' : 'text-orange-500') : 'text-slate-300';

    return (
        <div onClick={() => onClick(session)} className='group bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-5 sm:p-6 rounded-2xl sm:rounded-[2rem] flex flex-col md:flex-row items-center gap-4 transition-all hover:shadow-lg dark:hover:shadow-slate-900/50 active:scale-[0.98] cursor-pointer'>
            <div className='flex items-center gap-4 sm:gap-6 w-full md:w-auto flex-grow'>
                ...
                <div className='overflow-hidden'>
                    <h3 className='font-bold text-slate-900 dark:text-slate-100 text-base sm:text-lg truncate group-hover:text-teal-600 dark:group-hover:text-teal-400'>{session.role}</h3>
                    <div className='flex items-center gap-2 text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-tight'>
                        <span>{new Date(session.createdAt).toLocaleDateString()}</span>
                        <span>.</span>
                        <span className='text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded-md'>{session.level}</span>
                    </div>
                </div>
            </div>
            ...
            <div className='hidden md:block w-px h-10 bg-slate-100 dark:bg-slate-800 mx-2'></div>

            <button onClick={(e) => { e.stopPropagation(); if (isDeletable) onDelete(e, session._id) }} className='p-3 text-slate-300 dark:text-slate-600 hover:text-rose-500 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-xl transition-all' title='Delete Session'>
